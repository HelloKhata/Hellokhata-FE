...
export default function NewExpensePage({ onSuccess, onClose, isModal = false }: NewExpensePageProps = {}) {
  ...
  const formContent = (
    <Card className={cn(isModal && "border-0 shadow-none bg-transparent p-0")}>
      {!isModal && (
        <CardHeader>
      ...
      <CardContent className={cn("space-y-5", isModal && "p-0")}>
      ...
      <CardFooter className={cn("flex gap-3", isModal && "px-0 pb-0 pt-4")}>
        <Button
